...
grep -Fq 'Verify command: docker ps --filter name=shared-postgres' "$WORKFLOW_FILE"
grep -Fq 'docker ps --filter name=shared-postgres' "$WORKFLOW_FILE"
...
if rg -n 'docker compose -f /opt/infra/postgresql/docker-compose\.yml ps' "$WORKFLOW_FILE"; then
    echo "postgres deploy verification must not re-read the secret-backed compose file without env" >&2
    exit 1
fi
